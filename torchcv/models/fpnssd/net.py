...
class FPNSSD512(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        model_dir = '/home/jacobwang/torch_model/ResNet'
        params = torch.load(os.path.join(model_dir, 'resnet50-19c8e357.pth'))
        # params = torch.load(os.path.join(model_dir, 'resnet101-5d3b4d8f.pth'))
        # self.fpn = FPN101()
        self.fpn = FPN50()
        self.fpn.load_state_dict(params, strict=False)
...
def test():
    net = FPNSSD512(21)
    loc_preds, cls_preds = net(torch.randn(1,3,256,1024))
